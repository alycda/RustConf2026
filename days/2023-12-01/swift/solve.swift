...
//
// Run via: just days swift-demo 2023-12-01 (regenerates the header, builds
...
///
/// `Bundle.main.bundlePath`, not `#filePath`. `#filePath` looks like the
/// `__file__` python/solve.py uses and is not one: it is baked in at compile
/// time as *verbatim whatever path swiftc was given*, so a compiler invoked
/// with a relative path leaves a relative path in the binary, which then
/// resolves against whatever directory the binary happens to be run from.
/// Compiled as `swiftc solve.swift` and run from `/`, it claims to live in
/// `/`. `bundlePath` asks the runtime where the executable actually is, and
/// is absolute either way — the same answer Kotlin gets from
/// `codeSource.location` and Dart from `Platform.script`.
...
private func call(
    _ name: String,
    _ function: (UnsafePointer<CChar>?, UnsafeMutablePointer<UInt32>?) -> Int32,
    _ text: String
) -> UInt32 {
    var slot: UInt32 = 0
    ...
    guard status == 0 else {
        fail(
            "\(name) failed with status \(status) (-1: input was null or not valid UTF-8; "
                + "-2: the calibration total did not fit in a uint32_t)")
    }
    return slot
}
