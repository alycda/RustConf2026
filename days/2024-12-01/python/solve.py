...
header `just days bindgen 2024-12-01` generates (regenerated below if
...
Run via: just days python-demo 2024-12-01 (builds the header + cdylib
...
HEADER = DAY_DIR / "include" / "aoc_2024_12_01.h"


def header_declarations() -> str:
    if not HEADER.exists():
        try:
            subprocess.run(["just", "days", "bindgen", "2024-12-01"], cwd=REPO_ROOT, check=True)
        except FileNotFoundError:
            sys.exit(
                "no header and `just` not found — install just (>= 1.31), or generate it "
                "directly: cd days/2024-12-01 && mkdir -p include && "
...
def load_library():
    ...
    for profile in ("debug", "release"):
        for name in ("libaoc_2024_12_01.so", "libaoc_2024_12_01.dylib", "aoc_2024_12_01.dll"):
            candidate = DAYS_DIR / "target" / profile / name
            if candidate.exists():
                return ffi, ffi.dlopen(str(candidate))

    sys.exit("no libaoc_2024_12_01.{so,dylib} / aoc_2024_12_01.dll found — run: cd days && cargo build -p aoc-2024-12-01 --lib")
...
def call(ffi, fn, name: str, text: str) -> int:
    ...
    if status != 0:
        sys.exit(
            f"{name} failed with status {status} (-1: input was null, not valid UTF-8, "
...
def main() -> None:
    ...
    print(f"Part 2 🐍(🦀): {call(ffi, lib.aoc_2024_12_01_part2, 'part2', text)}")
