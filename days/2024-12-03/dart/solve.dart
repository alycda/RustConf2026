...
// hand-transcribed from include/aoc_2024_12_03.h below. (A generator,
// package:ffigen, exists for larger APIs — pulling it in for two
// functions would be more machinery than the thing it generates.)
//
// Run via: just days dart-demo 2024-12-03 (fetches ffi, builds the
...
typedef _PartFnNative = Int32 Function(
    Pointer<Utf8> input, Pointer<Uint64> outSum);
...
DynamicLibrary _loadLibrary() {
  ...
  for (final profile in ['debug', 'release']) {
    ...
    for (final name in ['libaoc_2024_12_03.so', 'libaoc_2024_12_03.dylib', 'aoc_2024_12_03.dll']) {
  ...
  stderr.writeln(
      'no libaoc_2024_12_03.{so,dylib} / aoc_2024_12_03.dll found — run: cd days && cargo build -p aoc-2024-12-03 --lib');
  exit(1);
}
...
int _call(_PartFnDart fn, String name, String text) {
  ...
  final outSum = calloc<Uint64>();
  int status;
  int sum;
  try {
    status = fn(inputPtr, outSum);
    sum = outSum.value;
  } finally {
...
void main() {
  ...
  final part1 =
      lib.lookupFunction<_PartFnNative, _PartFnDart>('aoc_2024_12_03_part1');
  final part2 =
      lib.lookupFunction<_PartFnNative, _PartFnDart>('aoc_2024_12_03_part2');

  final inputFile = File('${_dayDir().parent.parent.path}/inputs/2024-12-03.txt');
