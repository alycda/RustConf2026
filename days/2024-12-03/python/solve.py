...
header `just days bindgen 2024-12-03` generates (regenerated below if
...
Run via: just days python-demo 2024-12-03 (builds the header + cdylib
...
HEADER = DAY_DIR / "include" / "aoc_2024_12_03.h"


def header_declarations() -> str:
    if not HEADER.exists():
        try:
            subprocess.run(["just", "days", "bindgen", "2024-12-03"], cwd=REPO_ROOT, check=True)
        except FileNotFoundError:
            sys.exit(
                "no header and `just` not found — install just (>= 1.31), or generate it "
                "directly: cd days/2024-12-03 && mkdir -p include && "
                "cbindgen --config cbindgen.toml --output include/aoc_2024_12_03.h src/c_api.rs"
            )
...
def load_library():
    ...
    # A cdylib takes the host's name, not Rust's choice: libaoc_2024_12_03.so on
    # Linux, libaoc_2024_12_03.dylib on macOS, aoc_2024_12_03.dll (no lib prefix) on
    # Windows. Searching the three filenames needs no platform check —
    # whichever one cargo produced is the one that exists.
    for profile in ("debug", "release"):
        for name in ("libaoc_2024_12_03.so", "libaoc_2024_12_03.dylib", "aoc_2024_12_03.dll"):
            candidate = DAYS_DIR / "target" / profile / name
            if candidate.exists():
                return ffi, ffi.dlopen(str(candidate))

    sys.exit("no libaoc_2024_12_03.{so,dylib} / aoc_2024_12_03.dll found — run: cd days && cargo build -p aoc-2024-12-03 --lib")
...
def main() -> None:
    ...
    input_path = REPO_ROOT / "inputs" / "2024-12-03.txt"
    if not input_path.exists():
        sys.exit(f"no puzzle input at {input_path} (see .gitignore)")
    text = input_path.read_text()

    print(f"Part 1 🐍(🦀): {call(ffi, lib.aoc_2024_12_03_part1, 'part1', text)}")
    print(f"Part 2 🐍(🦀): {call(ffi, lib.aoc_2024_12_03_part2, 'part2', text)}")
