...
//                 typing and the least checking of the four (this day's
//                 uint64_t crosses it as a signed Long, and nothing says so
//                 but a comment).
//   Swift (here)  needs no signature at all. `module.modulemap` points clang
//                 at the generated header, `import AocDay` brings the two
//                 functions in as ordinary typed Swift functions — UInt64
//                 out-params included, because the header said uint64_t —
//                 and the compiler checks every call against the real
//                 header. This is the only track where a header that
//                 changed shape is a build failure rather than something
//                 you find out about at runtime, if at all.
//
...
//
// Run via: just days swift-demo 2024-12-03 (regenerates the header, builds
...
private func call(
    _ name: String,
    _ function: (UnsafePointer<CChar>?, UnsafeMutablePointer<UInt64>?) -> Int32,
    _ text: String
) -> UInt64 {
    var slot: UInt64 = 0
...
let inputPath = daysDir.deletingLastPathComponent().appendingPathComponent("inputs/2024-12-03.txt")
guard let text = try? String(contentsOf: inputPath, encoding: .utf8) else {
    fail("no puzzle input at \(inputPath.path) (see .gitignore)")
}

print("Part 1 🕊️(🦀): \(call("part1", aoc_2024_12_03_part1, text))")
print("Part 2 🕊️(🦀): \(call("part2", aoc_2024_12_03_part2, text))")
