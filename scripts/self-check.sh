...
# Godot, and wasm — Exercise 4's, node plus the wasm32 target).
...
#        ./scripts/self-check.sh --track <swift|kotlin|python|dart|fortran|r|godot|wasm>
#
...
if [ "${1:-}" = "--track" ]; then
  case "${2:-}" in
    ...
    *) echo "unknown track '${2:-}' — one of: swift kotlin python dart fortran r godot wasm" >&2; exit 2 ;;
  esac
fi
